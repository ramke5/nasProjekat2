...
<%@include file="/WEB-INF/includes/header.jsp" %>
    ...
    <div class="container-fluid">
        ...
        <div>
            <div class="row-fluid">
                <div class="span6">
                    <a class="btn blue" href="/user/labUser/create?labId=${labObj.id}"><i class="icon-plus-sign"></i> Dodaj novog korisnika laboratorije</a>
                </div>
                <div class="span6">
                    <form name="search">
            ...
            <div class="row-fluid">
                <div class="span12">
                    <display:table list="${data}" uid="row" class="table" sort="external" requestURI="" partialList="true" size="${total}" pagesize="10">
                        ...
                        <c:if
                                test="${row.enabled == false}"><c:out value="Ne"/></c:if></display:column>
                        <display:column > <i class="icon-edit icon-grey"></i>
                            <a href="/user/labUser/edit/${row.id}"> Izmijeni korisnika</a> | <i class="icon-remove icon-grey"></i><a href="javascript:deleteItem((${row.id}))"> Obriši korisnika</a>
                        </display:column>
                    </display:table>
                </div>
            </div>
    ...
    <form name="delForm" action="/user/labUser/delete" method="post">
...
<script>
    $(document).ready(function () {
        App.init();
        App.setActiveNavigation('/user/labUser/list');
