...
var App = function () {
    ...
    changePageSize:function(count){
        var newURL = window.location.href;
        var queryParam= this.getParameterByName('q',newURL);
        var urlWithOutParams =  newURL.split( '?' )[0];
        var submitUrl="";
        newURL = this.updateURLParameter(urlWithOutParams, 'limit', count.value);
        if(queryParam!=""){
            submitUrl = this.updateURLParameter(newURL, 'q',queryParam);
        }else
        {
            submitUrl = newURL;
        }
    ...
    fixContentHeight: function () {
        ...
        setActiveNavigation: function (path) {
            ...
            if (path == "") {
                $(".nav-collapse .start").addClass('active');
            } else {
                $(".nav-collapse  a").each(function () {
                    var href = $(this).attr('href');
                    if (href != '/' && href.substring(0, path.length) == path) {
                        var parentMenus = $(this).parentsUntil('.nav-collapse','li').addClass('active');
                        var topMenu = parentMenus.last();
                        topMenu.find($('.arrow')).addClass('open').append('<span class="selected"></span>');
                    }
                });
            }
        },

        activateSearchReset: function(){
        ...
        activateSessionTimeoutTracking: function(){
            $.idleTimeout('#idletimeout', '#idletimeout a', {
                idleAfter: 1800,   // 30 minutes    1800
               /* pollingInterval: 2,*/
