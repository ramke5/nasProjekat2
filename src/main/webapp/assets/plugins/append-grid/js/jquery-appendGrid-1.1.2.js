            ...
            if (target.length > 0) {
                ...
                if (isEmpty(settings.idPrefix)) {
                    ...
                    if (isEmpty(tbWhole.id) || tbWhole.id == '') {
                        ...
                        settings.idPrefix = 'lrTenant.'+tbWhole.id;
                    }
                }
                ...
                for (var z = 0; z < settings.totalColumns.length; z++) {
                    tbCell = tbRow.insertCell(-1);
                    $(tbCell).attr({style:'text-align:right'})
                    if(settings.totalColumns[z].name) {
                    	if (settings.totalColumns[z].name == "amount" || settings.totalColumns[z].name == "rate" || settings.totalColumns[z].name == "lltAndOSF" || settings.totalColumns[z].name == "baseYearSFRate" || settings.totalColumns[z].name == "passThroughs") {
                    		var totalId = settings.idPrefix + '_' + settings.totalColumns[z].name;
                            var totalDiv = document.createElement('div');
                            $(totalDiv).attr('class', 'input-prepend').css({ 'font-size': '1em', 'width': '99%', 'margin-right': '1%' }).appendTo(tbCell);
                            $('<span/>', { text: 'Total $', class: 'add-on' }).appendTo(totalDiv);
                            $('<input/>', { type: 'text', id: totalId, name: totalId, class: 'prependInputs decimal valid', readonly: 'readonly' })
                            	.css(settings.totalColumns[z].css)
                            	.appendTo(totalDiv);
                    	} else if (settings.totalColumns[z].name == "abtmtNoOfMonths" || settings.totalColumns[z].name == "monthTotal" || settings.totalColumns[z].name == "freeRentMonthsTotal") {
                    		var totalId = settings.idPrefix + '_' + settings.totalColumns[z].name;
                            var totalDiv = document.createElement('div');
                            $(totalDiv).attr('class', 'input-prepend').css({ 'font-size': '1em', 'width': '100%' }).appendTo(tbCell);
                            $('<span/>', { text: 'Total', class: 'add-on' }).appendTo(totalDiv);
                            $('<input/>', { type: 'text', id: totalId, name: totalId, class: 'prependInputs decimal valid', readonly: 'readonly' })
                            	.css(settings.totalColumns[z].css)
                            	.appendTo(totalDiv);
                    	} else {
                    		$(tbCell).append($('<span name="'+settings.idPrefix+'_'+settings.totalColumns[z].name+'"/>').text(''));
                        }
                	} else {
                    	$(tbCell).append($('<span/>'));
                    }
                    $(tbCell).addClass('row-order');
                }
                /*tbRow = tbFoot.insertRow(-1);
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span>').text("")).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span/>').text("")).attr({type:'text', id:settings.idPrefix +'_monthTotal',name:settings.idPrefix +'_monthTotal',style:'text-align:right'}).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span/>').text("")).attr({type:'text', id:settings.idPrefix +'_lltAndOSF',name:settings.idPrefix +'_monthTotal',style:'text-align:right'}).addClass('row-order');
                tbCell = tbRow.insertCell(-1);$(tbCell).append($('<span>').text("")).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span/>').text("")).attr({type:'text', id:settings.idPrefix +'_baseYearSFRate',name:settings.idPrefix +'_monthTotal',style:'text-align:right'}).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span/>').text("")).attr({type:'text', id:settings.idPrefix +'_amount',name:settings.idPrefix +'_amountTotal',style:'text-align:right'}).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span>').text("")).addClass('row-order');
                tbCell = tbRow.insertCell(-1);
                $(tbCell).append($('<span/>').text("")).attr({type:'text', id:settings.idPrefix +'_abtmtNoOfMonths',name:settings.idPrefix +'_monthTotal',style:'text-align:right'}).addClass('row-order');
                tbCell = tbRow.insertCell(-1);$(tbCell).append($('<span>').text("")).addClass('row-order');
                tbCell = tbRow.insertCell(-1);$(tbCell).append($('<span>').text("")).addClass('row-order');*/
                ...
                $(tbCell).append($('<button/>').attr({ type: 'button', title: settings.textResources.append }).button({ icons: { primary: 'ui-icon-plusthick' }, text: true }).click(function () { insertRow(tbWhole, 1, null, null); }));
...
//                $(tbCell).append($('<button/>').attr({ type: 'button', title: settings.textResources.removeLast }).button({ icons: { primary: 'ui-icon-closethick' }, text: false }).click(function () { removeRow(tbWhole, null, this.value, false); }));
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            for (var y = 0; y < settings.columns.length; y++) {
                ...
                if (settings.columns[y].type == 'custom') {
                    ...
                    ctrl.id = settings.idPrefix + '['+uniqueIndex+'].' + settings.columns[y].name;
                    ctrl.name = ctrl.id;
                    // Build option list
                ...
                if (settings.columns[y].type != 'custom') {
                    ...
                    if (typeof (settings.columns[y].onKeyUp) == 'function') $(ctrl).keyup({ caller: tbWhole, callback: settings.columns[y].onKeyUp, uniqueIndex: uniqueIndex }, function (evt) { evt.data.callback(evt, $(evt.data.caller).appendGrid('getRowIndex', evt.data.uniqueIndex)); });
                }
                // Set default value
         ...
         /*   $(tbCell).append($('<button/>').addClass('moveUp').val(uniqueIndex).attr({ id: settings.idPrefix + '_MoveUp_' + uniqueIndex, type: 'button', title: settings.textResources.moveUp, tabindex: -1 }).button({ icons: { primary: 'ui-icon-arrowthick-1-n' }, text: false }).click(function () {
            ...
            for (var y = 0; y < hidden.length; y++) {
                ctrl = document.createElement('input');
                ctrl.id = settings.idPrefix + '['+uniqueIndex+'].' + settings.columns[hidden[y]].name;
                ctrl.type = 'hidden';
                tbCell.appendChild(ctrl);
            }
        }
    ...
    function getCellCtrl(type, idPrefix, columnName, uniqueIndex) {
        return document.getElementById(idPrefix + '['+uniqueIndex+'].' + columnName);
    }
